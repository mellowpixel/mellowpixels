...
function sydney_custom_styles($custom) {
	...
	if ( (get_theme_mod('front_header_type','slider') == 'nothing' && is_front_page()) || (get_theme_mod('site_header_type') == 'nothing' && !is_front_page()) ) {
		...
		// $custom .= ".site-header { position:fixed; background-color: rgba(227,23,70,1);}" . "\n";
		$custom .= ".admin-bar .site-header,.admin-bar .site-header.float-header { top:0;}"."\n";
		$custom .= ".site-header.fixed {position:fixed;}"."\n";
	...
	if ($sticky_menu == 'static') {
		$custom .= ".site-header.fixed { position: static;}"."\n";
	}
	...
	// $custom .= "@media only screen and (max-width: 1024px) { .site-header { background-color: rgba(227,23,70,1);}}" . "\n";
	//Site title
	...
	//Footer background
	$footer_background = get_theme_mod( 'footer_background', '#00A3DE' );
